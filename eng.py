...
async def Sikandar(update: Update, context: CallbackConte@God_x_Sikandarxt):
    chat_id = update.effective_chat.id
    args = context.args
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Incorrect usage! Use: /Sikandar <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
